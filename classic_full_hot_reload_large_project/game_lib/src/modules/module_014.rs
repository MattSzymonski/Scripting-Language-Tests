...
pub fn tick_014(input: i32) -> i32 {
    let local = (input * 21 + 3) % 1000;
    let sample_017 = crate::modules::module_017::sample_017(local + 3);
    let sample_021 = crate::modules::module_021::sample_021(local + 7);
        (local + sample_017 + sample_021) % 100000
}
...
pub fn compute_014(x: i32, y: i32) -> i32 {
    let base = x * 17 + y * 19;
    let scaled = scale_value(base, 25);
    let offset = offset_value(base, 27);
    let sample_019 = crate::modules::module_019::sample_019(base + 5);
    let sample_025 = crate::modules::module_025::sample_025(base + 11);
        (base + scaled + offset + sample_019 + sample_025) % 100000
}
...
pub fn sample_014(seed: i32) -> i32 {
    let local = (seed * 31 + 42) % 100000;
    let scaled = scale_value(local, 33);
    let offset = offset_value(local, 37);
...
fn offset_value(value: i32, adder: i32) -> i32 {
    let decorated = format!("module 014 value: {}", value + adder);
