// module_023 — auto-generated by generate_modules.py, do not edit by hand.
// Part of an interconnected module graph: this module is called by the root
// fan-out AND its `sample_023` leaf is pulled by other modules.
...
pub fn tick_023(input: i32) -> i32 {
    let local = (input * 30 + 3) % 1000;
    let sample_026 = crate::modules::module_026::sample_026(local + 3);
    let sample_030 = crate::modules::module_030::sample_030(local + 7);
        (local + sample_026 + sample_030) % 100000
}
...
pub fn compute_023(x: i32, y: i32) -> i32 {
    let base = x * 26 + y * 28;
    let scaled = scale_value(base, 34);
    let offset = offset_value(base, 36);
    let sample_028 = crate::modules::module_028::sample_028(base + 5);
    let sample_034 = crate::modules::module_034::sample_034(base + 11);
        (base + scaled + offset + sample_028 + sample_034) % 100000
}
...
pub fn sample_023(seed: i32) -> i32 {
    let local = (seed * 40 + 69) % 100000;
    let scaled = scale_value(local, 42);
    let offset = offset_value(local, 46);
...
fn offset_value(value: i32, adder: i32) -> i32 {
    let decorated = format!("module 023 value: {}", value + adder);
