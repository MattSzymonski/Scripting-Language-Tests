// module_026 — auto-generated by generate_modules.py, do not edit by hand.
// Part of an interconnected module graph: this module is called by the root
// fan-out AND its `sample_026` leaf is pulled by other modules.
...
pub fn tick_026(input: i32) -> i32 {
    let local = (input * 33 + 3) % 1000;
    let sample_029 = crate::modules::module_029::sample_029(local + 3);
    let sample_033 = crate::modules::module_033::sample_033(local + 7);
        (local + sample_029 + sample_033) % 100000
}
...
pub fn compute_026(x: i32, y: i32) -> i32 {
    let base = x * 29 + y * 31;
    let scaled = scale_value(base, 37);
    let offset = offset_value(base, 39);
    let sample_031 = crate::modules::module_031::sample_031(base + 5);
    let sample_037 = crate::modules::module_037::sample_037(base + 11);
        (base + scaled + offset + sample_031 + sample_037) % 100000
}
...
pub fn sample_026(seed: i32) -> i32 {
    let local = (seed * 43 + 78) % 100000;
    let scaled = scale_value(local, 45);
    let offset = offset_value(local, 49);
...
fn offset_value(value: i32, adder: i32) -> i32 {
    let decorated = format!("module 026 value: {}", value + adder);
