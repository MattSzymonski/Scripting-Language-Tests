// module_036 — auto-generated by generate_modules.py, do not edit by hand.
// Part of an interconnected module graph: this module is called by the root
// fan-out AND its `sample_036` leaf is pulled by other modules.
...
pub fn tick_036(input: i32) -> i32 {
    let local = (input * 43 + 3) % 1000;
    let sample_039 = crate::modules::module_039::sample_039(local + 3);
    let sample_043 = crate::modules::module_043::sample_043(local + 7);
        (local + sample_039 + sample_043) % 100000
}
...
pub fn compute_036(x: i32, y: i32) -> i32 {
    let base = x * 39 + y * 41;
    let scaled = scale_value(base, 47);
    let offset = offset_value(base, 49);
    let sample_041 = crate::modules::module_041::sample_041(base + 5);
    let sample_047 = crate::modules::module_047::sample_047(base + 11);
        (base + scaled + offset + sample_041 + sample_047) % 100000
}
...
pub fn sample_036(seed: i32) -> i32 {
    let local = (seed * 53 + 108) % 100000;
    let scaled = scale_value(local, 55);
    let offset = offset_value(local, 59);
...
fn offset_value(value: i32, adder: i32) -> i32 {
    let decorated = format!("module 036 value: {}", value + adder);
