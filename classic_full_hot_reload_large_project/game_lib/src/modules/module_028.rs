// module_028 — auto-generated by generate_modules.py, do not edit by hand.
// Part of an interconnected module graph: this module is called by the root
// fan-out AND its `sample_028` leaf is pulled by other modules.
...
pub fn tick_028(input: i32) -> i32 {
    let local = (input * 35 + 3) % 1000;
    let sample_031 = crate::modules::module_031::sample_031(local + 3);
    let sample_035 = crate::modules::module_035::sample_035(local + 7);
        (local + sample_031 + sample_035) % 100000
}
...
pub fn compute_028(x: i32, y: i32) -> i32 {
    let base = x * 31 + y * 33;
    let scaled = scale_value(base, 39);
    let offset = offset_value(base, 41);
    let sample_033 = crate::modules::module_033::sample_033(base + 5);
    let sample_039 = crate::modules::module_039::sample_039(base + 11);
        (base + scaled + offset + sample_033 + sample_039) % 100000
}
...
pub fn sample_028(seed: i32) -> i32 {
    let local = (seed * 45 + 84) % 100000;
    let scaled = scale_value(local, 47);
    let offset = offset_value(local, 51);
...
fn offset_value(value: i32, adder: i32) -> i32 {
    let decorated = format!("module 028 value: {}", value + adder);
