// module_025 — auto-generated by generate_modules.py, do not edit by hand.
// Part of an interconnected module graph: this module is called by the root
// fan-out AND its `sample_025` leaf is pulled by other modules.
...
pub fn tick_025(input: i32) -> i32 {
    let local = (input * 32 + 3) % 1000;
    let sample_028 = crate::modules::module_028::sample_028(local + 3);
    let sample_032 = crate::modules::module_032::sample_032(local + 7);
        (local + sample_028 + sample_032) % 100000
}
...
pub fn compute_025(x: i32, y: i32) -> i32 {
    let base = x * 28 + y * 30;
    let scaled = scale_value(base, 36);
    let offset = offset_value(base, 38);
    let sample_030 = crate::modules::module_030::sample_030(base + 5);
    let sample_036 = crate::modules::module_036::sample_036(base + 11);
        (base + scaled + offset + sample_030 + sample_036) % 100000
}
...
pub fn sample_025(seed: i32) -> i32 {
    let local = (seed * 42 + 75) % 100000;
    let scaled = scale_value(local, 44);
    let offset = offset_value(local, 48);
...
fn offset_value(value: i32, adder: i32) -> i32 {
    let decorated = format!("module 025 value: {}", value + adder);
