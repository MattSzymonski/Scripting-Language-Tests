...
pub fn compute_001(x: i32, y: i32) -> i32 {
    let base = x * 4 + y * 6;
    let scaled = scale_value(base, 12);
    let offset = offset_value(base, 14);
...
pub fn sample_001(seed: i32) -> i32 {
    let local = (seed * 18 + 3) % 100000;
    let scaled = scale_value(local, 20);
    let offset = offset_value(local, 24);
...
fn offset_value(value: i32, adder: i32) -> i32 {
    let decorated = format!("module 001 value: {}", value + adder);
