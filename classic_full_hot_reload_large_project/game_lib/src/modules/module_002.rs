...
pub fn compute_002(x: i32, y: i32) -> i32 {
    let base = x * 5 + y * 7;
    let scaled = scale_value(base, 13);
    let offset = offset_value(base, 15);
...
pub fn sample_002(seed: i32) -> i32 {
    let local = (seed * 19 + 6) % 100000;
    let scaled = scale_value(local, 21);
    let offset = offset_value(local, 25);
...
fn offset_value(value: i32, adder: i32) -> i32 {
    let decorated = format!("module 002 value: {}", value + adder);
