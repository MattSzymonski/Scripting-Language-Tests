// module_030 — auto-generated by generate_modules.py, do not edit by hand.
// Part of an interconnected module graph: this module is called by the root
// fan-out AND its `sample_030` leaf is pulled by other modules.
...
pub fn tick_030(input: i32) -> i32 {
    let local = (input * 37 + 3) % 1000;
    let sample_033 = crate::modules::module_033::sample_033(local + 3);
    let sample_037 = crate::modules::module_037::sample_037(local + 7);
        (local + sample_033 + sample_037) % 100000
}
...
pub fn compute_030(x: i32, y: i32) -> i32 {
    let base = x * 33 + y * 35;
    let scaled = scale_value(base, 41);
    let offset = offset_value(base, 43);
    let sample_035 = crate::modules::module_035::sample_035(base + 5);
    let sample_041 = crate::modules::module_041::sample_041(base + 11);
        (base + scaled + offset + sample_035 + sample_041) % 100000
}
...
pub fn sample_030(seed: i32) -> i32 {
    let local = (seed * 47 + 90) % 100000;
    let scaled = scale_value(local, 49);
    let offset = offset_value(local, 53);
...
fn offset_value(value: i32, adder: i32) -> i32 {
    let decorated = format!("module 030 value: {}", value + adder);
