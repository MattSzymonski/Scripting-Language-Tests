// module_029 — auto-generated by generate_modules.py, do not edit by hand.
// Part of an interconnected module graph: this module is called by the root
// fan-out AND its `sample_029` leaf is pulled by other modules.
...
pub fn tick_029(input: i32) -> i32 {
    let local = (input * 36 + 3) % 1000;
    let sample_032 = crate::modules::module_032::sample_032(local + 3);
    let sample_036 = crate::modules::module_036::sample_036(local + 7);
        (local + sample_032 + sample_036) % 100000
}
...
pub fn compute_029(x: i32, y: i32) -> i32 {
    let base = x * 32 + y * 34;
    let scaled = scale_value(base, 40);
    let offset = offset_value(base, 42);
    let sample_034 = crate::modules::module_034::sample_034(base + 5);
    let sample_040 = crate::modules::module_040::sample_040(base + 11);
        (base + scaled + offset + sample_034 + sample_040) % 100000
}
...
pub fn sample_029(seed: i32) -> i32 {
    let local = (seed * 46 + 87) % 100000;
    let scaled = scale_value(local, 48);
    let offset = offset_value(local, 52);
...
fn offset_value(value: i32, adder: i32) -> i32 {
    let decorated = format!("module 029 value: {}", value + adder);
