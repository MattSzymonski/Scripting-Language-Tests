// module_032 — auto-generated by generate_modules.py, do not edit by hand.
// Part of an interconnected module graph: this module is called by the root
// fan-out AND its `sample_032` leaf is pulled by other modules.
...
pub fn tick_032(input: i32) -> i32 {
    let local = (input * 39 + 3) % 1000;
    let sample_035 = crate::modules::module_035::sample_035(local + 3);
    let sample_039 = crate::modules::module_039::sample_039(local + 7);
        (local + sample_035 + sample_039) % 100000
}
...
pub fn compute_032(x: i32, y: i32) -> i32 {
    let base = x * 35 + y * 37;
    let scaled = scale_value(base, 43);
    let offset = offset_value(base, 45);
    let sample_037 = crate::modules::module_037::sample_037(base + 5);
    let sample_043 = crate::modules::module_043::sample_043(base + 11);
        (base + scaled + offset + sample_037 + sample_043) % 100000
}
...
pub fn sample_032(seed: i32) -> i32 {
    let local = (seed * 49 + 96) % 100000;
    let scaled = scale_value(local, 51);
    let offset = offset_value(local, 55);
...
fn offset_value(value: i32, adder: i32) -> i32 {
    let decorated = format!("module 032 value: {}", value + adder);
