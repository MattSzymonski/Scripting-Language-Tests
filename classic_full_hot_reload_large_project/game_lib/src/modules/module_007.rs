...
pub fn compute_007(x: i32, y: i32) -> i32 {
    let base = x * 10 + y * 12;
    let scaled = scale_value(base, 18);
    let offset = offset_value(base, 20);
...
pub fn sample_007(seed: i32) -> i32 {
    let local = (seed * 24 + 21) % 100000;
    let scaled = scale_value(local, 26);
    let offset = offset_value(local, 30);
...
fn offset_value(value: i32, adder: i32) -> i32 {
    let decorated = format!("module 007 value: {}", value + adder);
