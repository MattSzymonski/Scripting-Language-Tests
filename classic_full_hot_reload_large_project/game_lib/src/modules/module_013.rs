...
pub fn tick_013(input: i32) -> i32 {
    let local = (input * 20 + 3) % 1000;
    let sample_016 = crate::modules::module_016::sample_016(local + 3);
    let sample_020 = crate::modules::module_020::sample_020(local + 7);
        (local + sample_016 + sample_020) % 100000
}
...
pub fn compute_013(x: i32, y: i32) -> i32 {
    let base = x * 16 + y * 18;
    let scaled = scale_value(base, 24);
    let offset = offset_value(base, 26);
    let sample_018 = crate::modules::module_018::sample_018(base + 5);
    let sample_024 = crate::modules::module_024::sample_024(base + 11);
        (base + scaled + offset + sample_018 + sample_024) % 100000
}
...
pub fn sample_013(seed: i32) -> i32 {
    let local = (seed * 30 + 39) % 100000;
    let scaled = scale_value(local, 32);
    let offset = offset_value(local, 36);
...
fn offset_value(value: i32, adder: i32) -> i32 {
    let decorated = format!("module 013 value: {}", value + adder);
