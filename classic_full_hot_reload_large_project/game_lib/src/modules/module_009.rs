...
pub fn compute_009(x: i32, y: i32) -> i32 {
    let base = x * 12 + y * 14;
    let scaled = scale_value(base, 20);
    let offset = offset_value(base, 22);
    let sample_014 = crate::modules::module_014::sample_014(base + 5);
    let sample_020 = crate::modules::module_020::sample_020(base + 11);
        (base + scaled + offset + sample_014 + sample_020) % 100000
}
...
pub fn sample_009(seed: i32) -> i32 {
    let local = (seed * 26 + 27) % 100000;
    let scaled = scale_value(local, 28);
    let offset = offset_value(local, 32);
...
fn offset_value(value: i32, adder: i32) -> i32 {
    let decorated = format!("module 009 value: {}", value + adder);
