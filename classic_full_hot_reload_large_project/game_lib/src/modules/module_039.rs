// module_039 — auto-generated by generate_modules.py, do not edit by hand.
// Part of an interconnected module graph: this module is called by the root
// fan-out AND its `sample_039` leaf is pulled by other modules.
...
pub fn tick_039(input: i32) -> i32 {
    let local = (input * 46 + 3) % 1000;
    let sample_042 = crate::modules::module_042::sample_042(local + 3);
    let sample_046 = crate::modules::module_046::sample_046(local + 7);
        (local + sample_042 + sample_046) % 100000
}
...
pub fn compute_039(x: i32, y: i32) -> i32 {
    let base = x * 42 + y * 44;
    let scaled = scale_value(base, 50);
    let offset = offset_value(base, 52);
    let sample_044 = crate::modules::module_044::sample_044(base + 5);
    let sample_000 = crate::modules::module_000::sample_000(base + 11);
        (base + scaled + offset + sample_044 + sample_000) % 100000
}
...
pub fn sample_039(seed: i32) -> i32 {
    let local = (seed * 56 + 117) % 100000;
    let scaled = scale_value(local, 58);
    let offset = offset_value(local, 62);
...
fn offset_value(value: i32, adder: i32) -> i32 {
    let decorated = format!("module 039 value: {}", value + adder);
