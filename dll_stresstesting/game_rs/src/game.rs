...
mod color {
    ...
    pub const SKY: u32 = rgba(115, 190, 240, 255);
    pub const SKY_DEEP: u32 = rgba(100, 175, 235, 255);
    ...
    pub const BIRD_GOLD: u32 = rgba(255, 205, 40, 255);
    pub const BIRD_ORANGE: u32 = rgba(235, 140, 30, 255);
}
...
fn draw_background(api: &EngineApi, screen_w: f32, screen_h: f32) {
    ...
    let clouds = [(90.0_f32, 120.0_f32, 26.0_f32), (340.0, 80.0, 20.0), (250.0, 200.0, 18.0)];
    for (cx, cy, r) in clouds {
        (api.draw_circle)(cx, cy, r, color::WHITE_CLOUD);
        (api.draw_circle)(cx + r, cy + 4.0, r * 0.8, color::WHITE_CLOUD);
        (api.draw_circle)(cx - r, cy + 6.0, r * 0.7, color::WHITE_CLOUD);
    }
}
...
fn draw_hud(state: &GameState, api: &EngineApi, screen_w: f32, screen_h: f32) {
    ...
    draw_score(state.score, api, screen_w);

    match state.state {
        STATE_READY => {
            draw_centered_text(api, "FLAPPY BIRD", screen_h * 0.30, 56.0, color::GOLD, screen_w);
            draw_centered_text(api, "Press SPACE / click to flap", screen_h * 0.42, 26.0, color::WHITE, screen_w);
            draw_centered_text(api, "ESC to quit", screen_h * 0.47, 22.0, color::WHITE, screen_w);
        }
        STATE_GAME_OVER => {
